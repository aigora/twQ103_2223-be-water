			...
			case '1':
				...
				do{
					...
					if(opcion2==1){
						...
						ph_max=maximopH(fuentes,nfuentes);
						printf("El pH maximo = %.2f\n",ph_max);
						return 0;
...
float maximopH(struct Tfuentes fuentes[],int nfuentes){
	...
	float pHmax;
	 ...
	 for ( i = 0; i < nfuentes; i++) {
        // Leemos la información de la fuente desde el archivo fentrada
        fscanf(fentrada, " %s %f", fuentes[i].nombre, &fuentes[i].pH);
        
        // Verificamos si el pH actual es mayor al máximo encontrado hasta ahora
        if (fuentes[i].pH > pHmax) {
            pHmax = fuentes[i].pH; // Actualizamos el valor máximo del pH
        }
    }
        return pHmax;
}
